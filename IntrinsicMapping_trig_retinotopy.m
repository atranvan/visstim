...
%
% Adapted from Bruno Pichleer's Grating Script
...
p.addParamValue('experimentType', 'D'); %mask for intrinsic imaging only works in triggered mode 
...
p.addParamValue('spaceFreqDeg',0.08);                        % spatial frequency in cycles / degree (for V1 0.02)
p.addParamValue('tempFreq',1);                               % temporal frequency in Hz  (for V1: 2)
...
p.addParamValue('mouseDistanceCm', 21);                       % mouse distance from the screen im cm
...
p.addParamValue('patchSubset', [1 1 1;1 1 1;1 1 1])              % patchGridX Y sized matrix defining which patches to show, good for intrinsic imaging mapping
