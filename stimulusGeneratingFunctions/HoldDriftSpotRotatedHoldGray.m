function  stimulusInfo = HoldDriftSpotRotatedHoldGray(q)
%HOLDDriftReverseDriftHOLDGrayscreen  This function displays static then drifting then static
% gratings, all in the same direction, for specified times. 
% The drifting switches direction (to opposite direction) in the middle
% Gray screen is displayed in between directions
%
% Inputs:
%
%   q
...
try
    ...
    for repeat = 1:q.repeats
        for d=1:q.directionsNum
            ...
            stimulusInfo.stimuli(currentStimIndex).startTime = toc;
            for frameCount = 1:round((2*q.driftTime+q.spotDriftTime) * q.hz);
                ...
                if ((frameCount > round(q.driftTime* q.hz)+1)&&(frameCount<round((q.driftTime+q.spotDriftTime)*q.hz)));
                    q.gratingcirc=q.gratingtex; 
                     ...
                     %xoffset = mod(frameCount*DG_ShiftPerFrame,DG_SpatialPeriod);
                     %srcRect=[xoffset 0 (xoffset + q.screenRect(3)*2) q.screenRect(4)*2];
                     %srcCircleRect=[xoffset 0 (xoffset + visiblesize*2) visiblesize*2];
                     ...
                     %Screen('DrawTexture', q.window, masktex, [0 0 visiblesize visiblesize], dstCircleRect, thisDirection);
