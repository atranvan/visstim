function stimulusInfo = DriftGrayTriggered(q)
% DRIFTGRAY Displays a moving grating followed by gray screen until
% next trigger
%
% Inputs:
%
...
try
    ...
    for repeat = 1:q.repeats
        for d=1:q.directionsNum
            ...
            %PostDrift gray screen - until terminated by a trigger
            ...
            while inputSingleScan(q.input)
                Screen('FillRect', q.window, q.grey);
             ...
             %Quit only if 'esc' key was presseds
            [~, ~, keyCode] = KbCheck;
            if keyCode(KbName('escape')), error('escape'), end
            if keyCode(KbName('t'))
