function  stimulusInfo = SpotHDHGBlackLMTriggered(q)
...
% The temporal structure is Hold/Drift/Hold/Black
% The position of the spot on the screen is an input variable 
% A landmark (here white disk on top right corner of screen) at a fixed position 
% is also displayed at all times
%
% Inputs:
%
%   q
...
% Ouput:
%   stimulusInfo
%       .experimentType         'SpotHDHGBlackLM'
%       .triggering             'on'
...
grey = 177.5;
%landmark location and size
dstCircleLM = [0 0 210 210];
dstCircleLM = CenterRectOnPoint(dstCircleLM,1690,240);
...
try
    ...
    for repeat = 1:q.repeats
        for d=1:q.directionsNum
            ...
            % first static grating
            for holdFrames = 1: round(q.preDriftHoldTime *q.hz)
                xoffset = mod(1*DG_ShiftPerFrame,DG_SpatialPeriod);
                ...
                Screen('FillOval', q.window, 255, dstCircleLM);
            ...
            for frameCount = 1:round((q.driftTime) * q.hz);
                ...
                % Draw photodiode area in one corner of the screen
                if q.photoDiodeRect(2)
                    Screen('FillRect', q.window, 100,q.photoDiodeRect ) 
                end
