...
p.addParamValue('filePath', 'C:\Users\tranvaa\Documents\MATLAB\visstim\stimfiles')

% add a status file path. This allows VisStimAlex to output its current
% status to a text file, for remote monitoring

p.addParamValue('statusFilePath', 'C:\Users\tranvaa\Documents\MATLAB\visstim\stimstatus')
...
% Screen parameters:
p.addParamValue('screenWidthCm', 52);                         % screen size in cm
...
p.addParamValue('XSpanDegrees',96); 
p.addParamValue('YSpanDegrees',78);
% Frequency tuning
...
try
    ...
    elseif strcmp(q.experimentType, 'spnG')
        [q.window,q.screenRect,q.ifi]=initScreen(5); %Returns a handle to the active screen, a rectangle representing size,
    else
